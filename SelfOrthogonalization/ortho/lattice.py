...
class Lattice:
    ...
    def frompath(self, sym, λ):
        """ Find what coordinate in the First Brillouin Zone corresponds to λ.
        ...
        sym: list[str]
            labels of each high-symmetry vertex in the desired path
        λ: float
            x∈[0,1] marking location along k-path

        Returns
        -------
        b: list[float] with length d
            the corresponding reciprocal coordinate

        """
        xv, bv = self.vertices(sym)

        # IDENTIFY WHICH VERTICES λ IS BETWEEN
        for i in range(len(xv)):
            if λ < xv[i]: break         # NOW λ IS BETWEEN xv[i-1] AND xv[i]

        # PICK OUT RECIPROCAL-SPACE COORDINATE
        δ = (λ - xv[i-1]) / (xv[i] - xv[i-1])
        b = δ*(bv[i] - bv[i-1]) + bv[i-1]
